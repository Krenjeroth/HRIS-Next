...
import { button } from '@/app/types/pds';
...
function SalaryGradeTabs() {
    ...
    const [title, setTitle] = useState<string>("Venue");
    ...
    useEffect(() => {
        ...
        async function getData() {
            ...
            const resp = await HttpService.post("search-venues", postData);
            if (resp != null) {
    ...
    const getDataById = async (id: number) => {
        try {
            const resp = await HttpService.get("venues/" + id);
    ...
    const onFormSubmit = async (
        ...
        try {
            ...
            if (process == "Add") {
                ...
                const resp = await HttpService.post("venues", postData);
                if (resp.status === 200) {
            ...
            // update
                ...
                const resp = await HttpService.patch("venues/" + id, postData)
                if (resp.status === 200) {
                    let status = resp.data.status;
            ...
            // delete
                ...
                const resp = await HttpService.delete("venues/" + id);
                if (resp.status === 200) {
                    let status = resp.data.status;
                    if (status === "Request was Successful") {
